...
int flag_arrow=0, flag_balloon=1, count_arrow=10, 
    count_balloon=10;
...
void main()
{
  ...
  int maxx = getmaxx() ;
  int maxy = getmaxy() ;
  int p=400, q=300, m=100, n=100, x=m, y=n, key, score=0, 
      finish= 0, level=1, l_flag=1;
...
getimage (x+25, y-65, x+66, y+65, bow);
         ...
         (q+25))
	{
		...
		else
		{
			...
			outtextxy (getmaxx()/2-220, getmaxy()/2-180,
                            "GAME OVER..");
			settextstyle (8,0,5);
	...
	if(count_arrow <= 0)
		...
		outtextxy (getmaxx()/2-100,getmaxy()/2-70,"NO MORE
           		ARROWS");

	if(count_balloon <= 0)
	
		outtextxy (getmaxx()/2-120, getmaxy()/2-70, "NO MORE
                     BALLOONS");
		outtextxy (getmaxx()/2-120, getmaxy()/2-20,
                     "YOUR SCORE IS : ");
		itoa (score, score1, 10);
	...
	if(level == 1)
		...
		outtextxy (getmaxx()/2-220, etmaxy()/2+20, "YOU NEED
                      TO PRACTICE MORE");
	
	if(level == 2)
	
		outtextxy (getmaxx()/2-70, getmaxy()/2+20, "WELL
                      PLAYED");
	
	if(level==3)
	
		outtextxy (getmaxx()/2-220, getmaxy()/2+20, "YOU ARE
                      AN EFFICIENT SHOOTER");

		outtextxy(getmaxx()/2-110, getmaxy()/2+50, "PRESS Q TO
                     QUIT");
...
void fly (int *x, int *y)
{
	int x1;
	char cnt_ball[5];
	putimage (*x-4*BALLOON_SIZE, *y-5*BALLOON_SIZE, balloon,
               COPY_PUT);
	delay (25);
...
void start()
{
	float octave[7] = { 130.81, 146.83, 164.81, 174.61, 196, 
                          220, 246.94};
	setbkcolor (0);
	settextstyle (7,0,0);
	...
	outtextxy (10,110, "2. You can move the bow UP and DOWN with 
                the help of arrow keys.");
	outtextxy (10,150, "3. Press right arrow key to shoot the
                 arrow.");
	outtextxy (10,190, "4. You will gain 50 points for each
                balloon shoot.");
	outtextxy (10,230, "5. First level has 10 arrows and 10 
                balloons.");
	outtextxy (10,270, "6. You require to score 100 points to
                enter the second level.");
	outtextxy (10,310, "7. Second level has 10 arrows and 8
                balloons.");
	outtextxy (10,350, "8. You require to score 200 points to 
                enter the third level.");
	outtextxy (10,390, "9. Third level has 10 arrows and 6 
                balloons.");
	settextstyle (7,0,1);
